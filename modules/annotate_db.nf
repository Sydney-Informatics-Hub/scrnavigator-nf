process ANNOTATE_DATABASE {
    ...
    ext input_size: { rds_path.size() + ( annotation_db ? annotation_db.size() : 0 ) + ( ensdb ? ensdb.size() : 0 ) }
    memory { 8.GB + task.ext.input_size.B * 10 }
    container "sydneyinformaticshub/scrnavigator-nf-annotate"

    input:
    tuple val(cohort_name), path(rds_path), val(species), val(min_cells_for_annotation), path(annotation_db), path(ensdb)
    ...
    annotate_db.R "${cohort_name}" "${rds_path}" annotation.csv "${ensdb}"
    """
}